...
| gemini-3.7-flash via Surplus               | 1-Click Edit retake cleanup (the default since 2026-09; gemini-3.1-pro was the previous default)           | About $0.008 list per 14 minute take, measured in the keep-only eval (`one-click-benchmark-review.md`)                                                                | `AI_CLEAN_MODEL` in `src/app/api/clean-transcript/route.ts`                                 |
| claude-opus-4.7 via Surplus                | AI overlays: editorial planning, scene design, revision, retiming, and the render review                   | Assumed $5.00 in, $25.00 out per 1M tokens (Anthropic's Opus 4.5+ list; Surplus has not been checked for this model). Frames sent to the reviewer bill as image input | `AI_DIRECT_MODEL`, `AI_OVERLAY_MODEL`, `AI_OVERLAY_REVIEW_MODEL` in `src/lib/studio/scene/` |
| gemini-3.1-flash-image (Google direct), 1K | AI overlay pictures                                                                                        | $0.067 per 1K image                                                                                                                                                   | `GEMINI_IMAGE_MODEL` in `src/lib/studio/scene/generate-scene-image.ts`                      |
...
| 1-Click Edit cleanup (gemini-3.7-flash)               | 1             | 1, up to 3 on transient failure                                                           | $0.004             | $0.008 (measured, 14 min)      | $0.03 (3 attempts)                                                 | 8% to 16%            |
| Overlay placement                                     | 1             | 1 mini pass                                                                               | $0.008             | $0.010                         | $0.02                                                              | 16%                  |
| AI overlay planning (direct, Opus)                    | 1             | 2 Opus passes: author, then editorial review; transcript up to 5k words                   | $0.06 (short take) | $0.12 (9k in, 3k out per pass) | $0.29 (8k output tokens)                                           | 120% to 580%         |
| AI overlay design (Opus, per moment)                  | 2             | 2 Opus passes per moment: design, then checked design                                     | $0.10              | $0.16                          | $0.41 (14k output tokens)                                          | 100% to 410%         |
| AI overlay picture (Google direct, 1K)                | 2             | 1 image call                                                                              | $0.067             |                                | $0.07                                                              | 67%                  |
| AI overlay revision (Opus)                            | 2             | revision plan (1.5k out) + checked design (8k out cap)                                    | $0.08              |                                | $0.30                                                              | 80% to 300%          |
| AI overlay retiming (Opus)                            | 1             | 1 short Opus pass (600 out cap)                                                           | $0.03              |                                | $0.04                                                              | 60% to 80%           |
| Overlay render review (Opus, up to 8 frames)          | 0             | 1 Opus pass reading the composited frames; an automatic repair adds a checked design pass | $0.10 per review   |                                | $0.26 with repair                                                  | unmetered            |
| Canvas ask (Chirpy on an idea)                        | 1             | router + 1 mini pass with 12 lines of thread                                              | $0.006             |                                | $0.015                                                             | 12% to 30%           |
...
| Reference analysis, Instagram                         | 2             | Apify + Deepgram                                                                          | $0.025             |                                | $0.05                                                              | 25%                  |
| Reference analysis, TikTok                            | 2             | 1 Apify run, captions read for free                                                       | $0.01              |                                | $0.05 (Deepgram fallback when a video has no captions)             | 10%                  |
| Voice sample, Instagram (per 3 min)                   | 1             | Graph media URL (free) + Deepgram; Apify only when Graph withholds the file               | $0.013             | $0.065 for 15 min (5 credits)  | $0.02 per credit with the Apify fallback                           | 26% to 40%           |
| Voice sample, TikTok (per 3 min)                      | 1             | 1 Apify run, captions read for free                                                       | $0.01              |                                | $0.02 with the Deepgram fallback                                   | 20%                  |
| Voice sample, YouTube                                 | 0             | 0 paid calls (captions)                                                                   | $0.00              |                                |                                                                    | 0%                   |
| Voice profile derivation                              | 0             | 1 mini pass over up to 8 transcripts                                                      | $0.004             |                                | $0.01                                                              | n/a (free)           |
...
The overlay lines are the new surprise, and they are estimates: the code sends
no per-request token telemetry for these passes, and the Opus rate above is
Anthropic's list, not Surplus' settlement (which has run 64% to 99% below
catalog for other models). At list, every Opus overlay action costs more than
the credit it charges on the monthly plan, and the render review costs about a
dime per call with nothing charged at all. Measure before repricing: log input
and output tokens per overlay pass (section 11), then read the Surplus invoice
for one week of overlay use.

The cleanup line was the previous surprise. At list, gemini-3.1-pro's 4,400 output
...
### 3b. Calls that cost money and charge no credit

Audited 2026-09-07 across every outbound provider call in `src/` and every
route the native app calls. Everything below spends without a credit; each has
a rate limit, and that limit is the only bound on the spend.

| Call                                                  | Where                                                             | Cost at list                                                        | Bound                |
| ----------------------------------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------- | -------------------- |
| Brain router                                          | `src/lib/brain/context/select-model.ts`, before most AI tasks     | 1 mini pass, 160 output tokens, about $0.001                        | 40 per hour per user |
| Overlay render review and auto repair                 | `src/lib/studio/scene/review-rendered.ts`                         | $0.10 per review, $0.26 with repair (Opus, up to 8 frames as input) | 24 per hour per user |
| Voice profile derivation                              | `src/lib/voice/derive.ts`                                         | 1 mini pass over up to 8 transcripts, under $0.01                   | 3 per hour per user  |
| Instagram import, file lookup                         | `src/lib/publish/server/instagram-import.ts`                      | 1 Apify run only when Graph withholds the file, about $0.005        | 8 per day per user   |
| Waitlist confirmation email                           | `src/app/api/waitlist/route.ts`                                   | Resend, free tier                                                   | none                 |
| Channel listings, oEmbed, YouTube and TikTok captions | publish list libs, `youtube-transcript.ts`, `tiktok-subtitles.ts` | $0 (platform APIs and public caption files)                         | platform quotas      |

Everything else that reaches a provider reserves a credit first: the 17
`PAID_ACTIONS` in `src/lib/billing/actions.ts`, the four `GENERATE_CREDITS`
routes, coaching, and training feedback. No route calls a provider outside
those paths, and the native app only ever calls these routes.
...
| Reference analysis                            | 2            | 1 for YouTube, 2 for Instagram, TikTok and web pages                                | Web page summaries move to the mini model. TikTok now reads captions first, so it costs about what YouTube does.     |
| Voice sample                                  | 1 per 3 min  | 1 per 3 min, rounded up, YouTube free                                               | Shipped 2026-09-07. Reserved on the platform's length, settled on Deepgram's heard duration. Derivation is free.     |
| Canvas ask                                    | 1            | 1                                                                                   | A mini pass. Shipped 2026-09-07.                                                                                     |
| AI overlay planning                           | 1            | 2, or 1 on a model a class below Opus                                               | Two Opus passes per call cost more than one credit at list. Decide after a week of measured tokens.                  |
| AI overlay design                             | 2 per moment | 3 per moment                                                                        | Same reasoning; the picture stays at 2.                                                                              |
| AI overlay revision                           | 2            | 2                                                                                   | Holds if the checked design pass stays under about 4k output tokens; measure.                                        |
| Overlay render review                         | 0            | 0, but cap repairs at 1 per design and count the review toward the design's credits | Today an unmetered Opus call with frames attached.                                                                   |

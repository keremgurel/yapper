...
const RECESS = "bg-[color:var(--sg-ink-100)] dark:bg-[color:var(--sg-ink-900)]";
const GLYPH = "text-muted-foreground";
const FOCUS = "outline-none focus-visible:ring-2 focus-visible:ring-ring";
const ACCENT = "var(--sg-accent)";

export type DeleteButtonSize = "sm" | "md";

/** Tile, panel and glyph dimensions per size. `sm` sits inline in a row of icon buttons. */
const SIZES = {
  sm: {
    tile: 32,
    panel: 64,
    root: "h-8 rounded-xl",
    trigger: "h-8 w-8 rounded-xl",
    panelClass: "gap-1.5 rounded-xl",
    circle: "h-5.5 w-5.5",
    glyph: 16,
    mark: 11,
  },
  md: {
    tile: 48,
    panel: 84,
    root: "h-12 rounded-2xl",
    trigger: "h-12 w-12 rounded-2xl",
    panelClass: "gap-2 rounded-2xl",
    circle: "h-7 w-7",
    glyph: 20,
    mark: 14,
  },
...
const CIRCLE = `grid place-items-center rounded-full bg-card transition-colors duration-200 hover:bg-background ${FOCUS} ${LIFT}`;
  ...
  return (
    <motion.div className="flex" variants={reduced ? undefined : circleMotion}>
      <motion.button
        ...
        transition={PRESS}
        className={cn(CIRCLE, dims.circle)}
      >
        <svg
          {...ICON}
          width={dims.mark}
...
> & {
  /** Accessible name of the trigger, e.g. "Delete Hooks". Defaults to "Delete". */
  label?: string;
  size?: DeleteButtonSize;
  disabled?: boolean;
  onConfirm?: () => void;
...
/**
 * A trash tile that opens into confirm / cancel in place, so no site needs
 * its own `window.confirm` or a "are you sure" row. `onConfirm` fires on the
 * check, `onCancel` on the cross or Escape.
 */
export function DeleteButton({
  ...
  return (
    <motion.div
      ...
      data-status={status}
      className={cn("relative", dims.root, SURFACE, GLYPH, className)}
    ...
    >
      <motion.button
        ...
        transition={TAP}
        className={cn(
          "relative z-10 grid place-items-center disabled:opacity-50",
          dims.trigger,
          FOCUS,
        )}
      >
        ...
        {open && (
          ...
          >
            <span
              aria-hidden
              className={cn(
                "absolute top-1/2 -left-1.25 z-20 h-2.5 w-1.5 -translate-y-1/2 [clip-path:polygon(100%_0,0_50%,100%_100%)]",
                RECESS,
              )}
            />
            <Circle

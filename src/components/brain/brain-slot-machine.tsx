 ...
 */
export default function BrainSlotMachine({ pillars }: { pillars: string[] }) {
  const { idea, spinning, error, pull } = useBrainSpin();
  ...
  return (
    <section className="sg-panel space-y-4 p-4 sm:p-5">
      <header className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-xs">
            A pillar, an angle and a format, dealt at random and written for
            you.
          </p>
        </div>
      ...
      {pillars.length > 1 && (
        <div className="flex flex-wrap items-center gap-1.5">
          <span className="text-muted-foreground text-xs">Hold a pillar:</span>
          {pillars.map((pillar) => (
            <button
              key={pillar}
              type="button"
      ...
      {idea && (
        <article className="sg-sunken space-y-3 p-4">

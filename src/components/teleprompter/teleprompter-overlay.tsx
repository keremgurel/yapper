"use client";

import type { RefObject } from "react";

/** The scrolling teleprompter text, laid over the top of the camera. Render-only
 * — the scroll position is driven by useTeleprompterScroll via `scrollRef`. Size,
 * height, and shade come from the live teleprompter settings. */
export default function TeleprompterOverlay({
  scrollRef,
  text,
  fontScale = 1,
  heightPct = 44,
  opacity = 0.75,
}: {
  scrollRef: RefObject<HTMLDivElement | null>;
  text: string;
  fontScale?: number;
  heightPct?: number;
  opacity?: number;
}) {
  return (
    <div
      className="pointer-events-none absolute inset-x-0 top-0 z-30"
      style={{ height: `${heightPct}%` }}
    >
      {/* Readability scrim so white text survives a bright shot. */}
      <div
        className="absolute inset-0 bg-gradient-to-b from-black via-black/60 to-transparent"
        style={{ opacity }}
      />
      <div
        ref={scrollRef}
        className="relative h-full overflow-hidden px-6 py-6 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      >
        <p
          className="mx-auto max-w-2xl text-center leading-snug font-black whitespace-pre-line text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.85)]"
          style={{ fontSize: `${1.75 * fontScale}rem` }}
        >
          {text}
        </p>
      </div>
    </div>
  );
}
